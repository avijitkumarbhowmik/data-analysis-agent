...
### Phase 2 — Transparency, Charts & Live Feedback

- **Goal:** Every answer becomes richer and auditable — the agent renders an interactive chart when appropriate, flags data-quality issues, suggests 2–3 follow-up questions, streams the answer as it composes, shows an estimated cost per query, remembers the conversation across turns within a workspace, and persists a revisitable run history.
...
- **Independent slices (parallel build units):**
  - `backend-enrich` (backend) — real `enrich` graph node: chart-spec generation, data-quality profiling, follow-up suggestion, cost estimation. **Deps: none** (new modules under `src/analysis/`, one graph node wired real).
  - `backend-history-conversation` (backend) — run-history API, streaming (SSE) ask endpoint, and conversation-memory (message history in state + runner + prompts). **Deps: none** (different files: `src/api/history.py`, `src/api/ask.py` stream path, `src/graph/runner.py` memory).
  - `frontend-transparency` (frontend) — chart rendering, data-quality badges, follow-up chips, streamed answer, cost display, multi-turn chat thread, run-history panel. **Deps: none** (API contract).
- **Key surfaces / files:** `src/analysis/charts.py`, `src/analysis/quality.py`, `src/analysis/cost.py`, `src/api/history.py`, `src/api/ask.py` (stream), `src/graph/nodes.py` (enrich real), `frontend/src/app/components/*`, `tests/phase2/`, `frontend/tests/e2e/phase2.spec.ts`
- **Gate command:** `uv run --native-tls pytest tests/phase2 -q` (real Gemini via `.env`, SQLite) then `cd frontend && pnpm exec playwright test tests/e2e/phase2.spec.ts`. Tests assert on real shapes: a chart spec is produced for a "trend over time" question, at least one data-quality flag is raised on a fixture with injected nulls/dupes, ≥2 follow-ups returned, cost `> 0`, a second turn resolves a pronoun from the first turn's context, and a run persists and is retrievable via the history endpoint.
- **How the user tests it (handoff seed):** `cd frontend && pnpm build`, then `uv run python -m src`, open `http://localhost:8001/app/`. Ask a "show me X over time" question → a chart renders below the answer and streams in; a cost badge appears; 2–3 follow-up chips appear (click one → it asks with prior context). Data-quality flags show on a messy dataset. Open the **Run history** panel and click a past run to revisit its query + code + result. **Newly real:** charts, quality flags, follow-ups, streaming, cost, conversation memory, history. **Still stubbed:** multi-file joins, Excel sheet picker, column notes, save-derived, export/download.
